...
def extract_phonemes(filename):
    ...
    if Path(filename.replace(".txt", ".phones")).is_file():
        return
...
def extract_dir(root, kind):
    ...
    abs_paths=[]
    # for dirpath, _, filenames in tqdm(os.walk(root)):
    #     for f in filenames:
    #         abs_path = os.path.abspath(os.path.join(dirpath, f))
    #         if abs_path.endswith(ext):
    #              abs_paths.append(abs_path)
    for dirpath in get_files(root,ext,recurse=True):
        abs_paths.append(str(dirpath))
            
    with Pool(cpu_count()) as pool:
        r = list(tqdm(pool.imap(extraction_function,abs_paths),total=len(abs_paths)))
